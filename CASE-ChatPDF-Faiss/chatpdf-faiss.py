...
def extract_text_with_page_numbers(pdf) -> Tuple[str, Dict[int, str]]:
    ...
    返回:
        text: 提取的文本内容
        page_texts: 每页对应的文本内容字典，格式为 {页码: 文本内容}
    """
    text = ""
    page_texts = {}
...
def process_text_with_splitter(text: str, page_texts: Dict[int, str], save_path: str = None) -> FAISS:
    """
    处理文本并创建向量存储

    参数:
        text: 提取的文本内容
        page_texts: 每页对应的文本内容字典
    ...
    for chunk in chunks:
        ...
        if chunk_pages:
            ...
            # 如果找不到匹配（可能因为分块时的重叠或修改），使用模糊匹配
            max_overlap = 0
            best_page = None
            for page_num, page_text in page_texts.items():
                # 计算最长公共子串或相似度
                overlap_score = len(set(chunk.split()) & set(page_text.split()))
                if overlap_score > max_overlap:
                    max_overlap = overlap_score
                    best_page = page_num

            page_info[chunk] = best_page if best_page is not None else "未知"
...
# 读取PDF文件
pdf_reader = PdfReader('./2-NotebookLM使用.pdf')
# 提取文本和页码信息
text, page_texts = extract_text_with_page_numbers(pdf_reader)
# print(text, page_texts)
...
# 处理文本并创建知识库，同时保存到磁盘
save_dir = "./vector_db"
knowledgeBase = process_text_with_splitter(text, page_texts, save_path=save_dir)
'''
# 示例：如何加载已保存的向量数据库
